...
document.addEventListener('DOMContentLoaded', function onload() {
  ...
  var ANDROID_DB_FILE = 'apns_full_conf.xml';
  ...
  function queryAndroidDB(mcc, mnc) {
    ...
    while (res) { // turn each resulting XML element into a JS object
      ...
      for (var i = 0; i < res.attributes.length; i++) {
        ...
        switch (name) {
          case 'type': // array of comma-separated values
            apn[name] = value.split(',');
            break;
          case 'mvno_type': // mvno_type must be in lowercase
            apn[name] = value.toLowerCase();
            break;
          default: // all other attributes are plain strings
            apn[name] = value;
            break;
        }
      }
      if (!apn.hasOwnProperty('type')) { // type="*" if there's no type speficied
        apn.type = ['default', 'mms', 'supl', 'ims', 'dun', 'fota', 'ia', 'xcap'];
      }
      found.push(apn);
  ...
  function getBitmaskForTech(radioTech) {
    if (radioTech >= 1) {
  ...
  function getBitmaskFromString(bearerList) {
    var bearers = bearerList.split('|');
    var bearerBitmask = 0;
    ...
    bearers.forEach((bearer) => {
      ...
      try {
        bearerInt = parseInt(bearer.trim());
      } catch (e) {
        return 0;
      }
    ...
    for (var mcc = 1; mcc < 999; mcc++) {
     ...
     if (result && result.length) {
        ...
        for (var i = 0; i < result.length; i++) {
          // Convert bearer_bitmask to bearerBitmask, then delete the origin.
          if (result[i].bearer_bitmask) {
            var bearerBitmask = getBitmaskFromString(result[i].bearer_bitmask);

            result[i].bearer = bearerBitmask;
            delete(result[i].bearer_bitmask);
          }
