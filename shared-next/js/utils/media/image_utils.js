...
(function (exports) { // eslint-disable-line
  ...
  ImageUtils.getSizeAndType = function getSizeAndType(imageBlob) {
     ...
     */
    function findSizeAndType(imageBlob, amountToRead, success, failure) { // eslint-disable-line
      ...
      function parseImageData(buffer) {
        ...
        if (
          ...
          header[2] === 0x46 /* F */ &&
          header[3] === 0x38 /* 8 */ &&
          (header[4] === 0x37 /* 7 */ || header[4] === 0x39) /* Or 9 */ &&
          header[5] === 0x61
        ) {
          /* A */
          // This is a GIF file
          ...
          // This is a BMP file
          try {
            let width = view.getUint32(18, true); // 32-bit little endian width
            let height = view.getUint32(22, true); // 32-bit little endian height
            if (view.getUint16(14, true) === 12) {
          ...
          try {
            ...
            for (;;) {
               ...
               */
              if (
                (segmentType >= 0xc0 && segmentType <= 0xc3) ||
   ...
   */
  ImageUtils.resizeAndCropToCover = function resizeAndCropToCover( // eslint-disable-line
    inputImageBlob,
    outputWidth,
    outputHeight,
    outputType,
    ...
    if (
      !outputWidth ||
      !isFinite(outputWidth) ||
      outputWidth <= 0 ||
      !outputHeight ||
      !isFinite(outputHeight) ||
      outputHeight <= 0
    ) {
