/*
  global VideoPlayer, DeviceCapabilityManager, galleryErrorLargeSrc,
  Downsample
*/
 ...
 */
function MediaFrame( // eslint-disable-line
  container,
  includeVideo,
  maxImageSize,
  autoHideProgressBar
  ...
  this.image.onload = () => (this.image.style.opacity = 1);
...
window.api.l10n.change(() => {
  ...
  const containers = document.querySelectorAll('.media-frame-container');
  for (let container of containers) { // eslint-disable-line
    const instance = MediaFrame.instancesToLocalize.get(container);
    if (instance) {
      instance.localize();
    }
  }
...
DeviceCapabilityManager.get('hardware.memory').then(
  mem => {
...
MediaFrame.computeMaxImageDecodeSize = mem => {
  if (!mem) {
    return 0;
  } else if (mem < 256) {
    // This is a Tarako-class device ultra low-end device.
    return 2 * 1024 * 1024; // 2 megapixels
  } else if (mem < 512) {
    // This is a low-end 256mb device
    /*
    ...
    // A mid-range 512mb device
    return 5 * 1024 * 1024; // 5 megapixels
  }
  /*
   * A high-end device with 1 gigabyte or more of memory
...
MediaFrame.prototype.usePreview = function usePreview(preview, width, height) {
   ...
   */
  if (this.minimumPreviewWidth && this.minimumPreviewHeight) {
    return (
      Math.max(preview.width, preview.height) >=
        Math.max(this.minimumPreviewWidth, this.minimumPreviewHeight) &&
      Math.min(preview.width, preview.height) >=
        Math.min(this.minimumPreviewWidth, this.minimumPreviewHeight)
    );
  }
  return true;
};
 ...
 */
MediaFrame.prototype.gotPreview = function gotPreview(
  previewblob,
  previewWidth,
  previewHeight
) {
  // Create a blob URL for the preview
  this.previewurl = URL.createObjectURL(previewblob);
  /*
...
MediaFrame.prototype.noPreview = function noPreview(blob, width, height) {
  this.previewurl = null;
  /*
   * Figure out whether we can downsample the fullsize image for
   * use as a preview
   */
  const previewSampleSize = this.computePreviewSampleSize(blob, width, height);
  ...
  if (previewSampleSize === Downsample.NONE) {
     ...
     */
    this.previewImageURL = this.imageurl + previewSampleSize;
    // Compute the preview size based on the downsample amount.
    this.previewWidth = previewSampleSize.scale(width);
    this.previewHeight = previewSampleSize.scale(height);
 ...
 */
MediaFrame.prototype.computeFullSampleSize = function computeFullSampleSize(
  blob,
  width,
  height
) {
   ...
   */
  let max = MediaFrame.maxImageDecodeSize || 0;
  if (this.maximumImageSize && (max === 0 || this.maximumImageSize < max)) {
    max = this.maximumImageSize;
  }
...
MediaFrame.prototype.computePreviewSampleSize = function computePreviewSampleSize(
  blob,
  width,
  height
) {
   ...
   */
  const portraitScale = Math.min(screenWidth / width, screenHeight / height);
  /*
   * To display the image in landscape, this is we need to scale it
   * this much
   */
  const landscapeScale = Math.min(screenHeight / width, screenWidth / height);
...
MediaFrame.prototype.displayImage = function displayImage( // eslint-disable-line
  blob,
  width,
  height,
  preview,
  ...
  if (MediaFrame.pendingPromise) {
    MediaFrame.pendingPromise.then(() => {
      this.displayImage(blob, width, height, preview, rotation, mirrored);
    });
    return;
  }
   ...
   */
  this.fullSampleSize = this.computeFullSampleSize(blob, width, height);
   ...
   */
  if (this.usePreview(preview, width, height)) {
    if (preview.start) {
      this.gotPreview(
      ...
      const storage = navigator.b2g.getDeviceStorage('pictures');
      const getReq = storage.get(preview.filename);
      getReq.onsuccess = () => {
        this.gotPreview(getReq.result, preview.width, preview.height);
      };
      getReq.onerror = () => {
        this.noPreview(blob, width, height);
      };
    }
  } else {
    this.noPreview(blob, width, height);
 ...
 */
MediaFrame.prototype._displayImage = function _displayImage( // eslint-disable-line
  url,
  width,
  height,
  bg
...
MediaFrame.prototype.localize = function localize() {
  ...
  let portrait = this.fullsizeWidth < this.fullsizeHeight;
  if (this.rotation === 90 || this.rotation === 270) {
    // If rotated sideways, the width and height are swapped.
    portrait = !portrait;
  }
  const timestamp = this.imageblob.lastModifiedDate;
  const orientation = window.api.l10n.get(
    portrait ? 'orientationPortrait' : 'orientationLandscape'
  );
  let label = '';
  if (timestamp) {
    const locale_entry = window.api.l10n.get('imageDescription', {
      orientation
    });
    if (!this.dtf) {
      this.dtf = new window.api.l10n.DateTimeFormat();
    }
    label = this.dtf.localeFormat(new Date(timestamp), locale_entry);
  } else {
    label = window.api.l10n.get('imageDescriptionNoTimestamp', {
      orientation
    });
  }
...
MediaFrame.prototype._switchToFullSizeImage = function _switchToFullSizeImage() {
...
MediaFrame.prototype._switchToPreviewImage = function _switchToPreviewImage() {
  /*
...
MediaFrame.prototype.displayVideo = function displayVideo( // eslint-disable-line
  videoblob,
  posterblob,
  width,
  height,
...
MediaFrame.prototype.clear = function clear() {
  ...
  this.previewImageURL = null;
  this.fullsizeWidth = null;
  this.fullsizeHeight = null;
...
MediaFrame.prototype.setPosition = function setPosition(zoom) {
  ...
  const dx = this.fit.left + (this.fit.width - this.itemWidth) / 2;
  const dy = this.fit.top + (this.fit.height - this.itemHeight) / 2;
  const sx = this.mirrored ? -this.fit.scale : this.fit.scale;
  const sy = this.fit.scale;
...
MediaFrame.prototype.computeFit = function computeFit() {
  if (!this.displayingImage) return;
  this.viewportWidth = this.container.offsetWidth;
  this.viewportHeight = this.container.offsetHeight;

  const scalex = this.viewportWidth / this.itemWidth;
  const scaley = this.viewportHeight / this.itemHeight;
  const scale = Math.min(Math.min(scalex, scaley), 1);

  // Set the image size and position
  const width = Math.floor(this.itemWidth * scale);
  const height = Math.floor(this.itemHeight * scale);
...
MediaFrame.prototype.reset = function reset() {
   ...
   */
  if (this.displayingImage && !this.displayingPreview && this.previewImageURL) {
  ...
  if (this.displayingVideo) this.video.setPlayerSize();
};
...
MediaFrame.prototype.resize = function resize() {
  ...
  const oldfit = this.fit; // The current image fit
   ...
   */
  if (
    Math.abs(oldfit.scale - oldfit.baseScale) < 0.01 ||
 ...
 * to animate the zoom.
 */
MediaFrame.prototype.zoom = function zoom(scale, fixedX, fixedY, time) { // eslint-disable-line
   ...
   */
  let photoX = fixedX - this.fit.left;
  let photoY = fixedY - this.fit.top;
  ...
  if (this.fit.width <= this.viewportWidth) {
    ...
    if (this.fit.left > 0) this.fit.left = 0;
  ...
  if (this.fit.height <= this.viewportHeight) {
    ...
    if (this.fit.top > 0) this.fit.top = 0;
...
MediaFrame.prototype.zoomFrame = function zoomFrame( // eslint-disable-line
  scale,
  fixedX,
  fixedY,
  time
  ...
  this.fit.scale = scale;
  const overflowx = Math.abs(this.fit.width - this.viewportWidth);
  const overflowy = Math.abs(this.fit.height - this.viewportHeight);
  let ratiol = Math.abs(this.fit.left / overflowx);
  let ratiot = Math.abs(this.fit.top / overflowy);
 ...
 */
MediaFrame.prototype.pan = function pan(dx, dy) {
...
MediaFrame.prototype.setMinimumPreviewSize = function setMinimumPreviewSize(
  w,
  h
) {
