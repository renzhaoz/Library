(function(window, undefined) {
  ...
  Locale.prototype.build = function L_build(callback) {
    ...
    var source = window.api.l10n._config.localeSources[this.id] || 'app';
    var appVersion = window.api.l10n._config.appVersion;
  ...
  window.api.l10n = {
    ...
    get: function get(id, ctxdata) {
      return window.api.l10n.ctx.get(id, ctxdata);
    },
    formatValue: function(id, ctxdata) {
      return window.api.l10n.ctx.formatValue(id, ctxdata);
    },
    formatEntity: function(id, ctxdata) {
      return window.api.l10n.ctx.formatEntity(id, ctxdata);
    },
    translateFragment: function (fragment) {
      return translateFragment.call(window.api.l10n, fragment);
    },
    setAttributes: setL10nAttributes,
    getAttributes: getL10nAttributes,
    change: function ready(callback) {
      return window.api.l10n.ctx.ready(callback);
    },
    once: function once(callback) {
      return window.api.l10n.ctx.once(callback);
    },
    get readyState() {
      return window.api.l10n.ctx.isReady ? 'complete' : 'loading';
    },
    language: {
      set code(lang) {
        window.api.l10n.ctx.requestLocales(lang);
      },
      get code() {
        return window.api.l10n.ctx.supportedLocales[0];
      },
      get direction() {
        return getDirection(window.api.l10n.ctx.supportedLocales[0]);
      }
    },
  ...
  window.api.l10n.ctx.ready(onReady.bind(window.api.l10n));

  window.api.l10n.ctx.addEventListener('notfounderror',
  ...
  if (DEBUG) {
    window.api.l10n.ctx.addEventListener('fetcherror',
      console.error.bind(console));
    window.api.l10n.ctx.addEventListener('parseerror',
      console.error.bind(console));
    window.api.l10n.ctx.addEventListener('resolveerror',
      console.error.bind(console));
  }
  ...
  if (window.document) {
    ...
    waitFor('interactive', init.bind(window.api.l10n, pretranslate));
  }

  function initObserver() {
    nodeObserver = new MutationObserver(onMutations.bind(window.api.l10n));
    nodeObserver.observe(document, moConfig);
  }

  function init(pretranslate) {
    if (pretranslate) {
      initResources.call(window.api.l10n);
    } else {
      ...
      initObserver();
      window.setTimeout(initResources.bind(window.api.l10n));
    }
  }
  ...
  function registerLocales(meta, extraLangs) {
    ...
    window.api.l10n._config.localeSources = locales[1];
  ...
  function onMetaInjected(node, meta) {
    switch (node.getAttribute('name')) {
      ...
      case 'defaultLanguage':
        ...
        break;
      case 'appVersion':
        window.api.l10n._config.appVersion = node.getAttribute('content');
        break;
    }
  }
