...
const SimplePhoneMatcher = {
  ...
  sanitizedNumber: function sanitizedNumber(number) {
    const join = this._formattingChars.join('|\\');
  ...
  generateVariants: function generateVariants(number) {
    const sanitizedNumber = this.sanitizedNumber(number);

    let variants = [];

    variants = variants.concat(
      this._internationalPrefixes(sanitizedNumber),
  ...
  bestMatch: function bestMatch(variants, matches) {
    ...
    matches.forEach((match, matchIndex) => {
      const indexes = [];
      match.forEach((number, localIndex) => {
        const sanitizedNumber = this.sanitizedNumber(number);

        variants.forEach(function matchVariant(variant) {
          if (
            variant.indexOf(sanitizedNumber) !== -1 ||
            sanitizedNumber.indexOf(variant) !== -1
          ) {
            const { length } = sanitizedNumber;
  ...
  _internationalPrefixes: function _internationalPrefixes(number) {
  ...
  _trunkPrefixes: function _trunkPrefixes(number) {
    const variants = [];

    const prefixesWithTrunk0 = [];
    const prefixesWithNoTrunk = [];
    this._countryPrefixes.forEach(prefix => {
      if (prefix.t) {
    ...
    const trunk0Join = prefixesWithTrunk0.join('|');
    const trunk0Regexp = new RegExp(`^\\+(${trunk0Join})`);
    this._internationalPrefixes(number).some(variant => {
      const match = variant.match(trunk0Regexp);
    ...
    const noTrunkJoin = prefixesWithNoTrunk.join('|');
    const noTrunkRegexp = new RegExp(`^\\+(${noTrunkJoin})`);
    this._internationalPrefixes(number).some(variant => {
      const match = variant.match(noTrunkRegexp);
    ...
    const withTrunkRegexp = new RegExp(`^(${this._trunkCodes.join('|')})`);
  ...
  _areaPrefixes: function _areaPrefixes(number) {
    const variants = [];

    Object.keys(this._areaCodeSwipe).forEach(country => {
      const re = new RegExp(`^\\+${country}`);

      this._internationalPrefixes(number).some(variant => {
        const match = variant.match(re);

        if (match) {
          const afterArea = 1 + country.length + this._areaCodeSwipe[country];
  ...
  _carrierPrefixes: function _carrierPrefixes(number) {
    if (this.mcc !== '724') {
      return [];
    }
...
window.SimplePhoneMatcher = SimplePhoneMatcher;
